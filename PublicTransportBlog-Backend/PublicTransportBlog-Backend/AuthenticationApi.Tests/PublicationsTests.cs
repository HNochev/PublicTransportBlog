...
namespace AuthenticationApi.Tests
{
    public class PublicationsTests
    {
        ...
        {
            var options = new DbContextOptionsBuilder<AppDbContext>().UseInMemoryDatabase("AllNewsReturnCorrectNumber").Options;
            var dbContext = new AppDbContext(options);

            dbContext.Publications.Add(new Publication { Title = "1", Description = "1", ImgUrl = "1", IsDeleted = false, Date = DateTime.Now, Id = Guid.NewGuid() });
            dbContext.Publications.Add(new Publication { Title = "2", Description = "2", ImgUrl = "2", IsDeleted = false, Date = DateTime.Now, Id = Guid.NewGuid() });

            dbContext.SaveChanges();
            var service = new PublicationController();
